  ...
  Future<void> _createDizanpayPayment(double amount, String method, String? phone) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Erro ao criar o pagamento: ${response.statusCode}')));
      }
    } catch (e) {
            ...
            builder: (context, billingState) {
              return BlocBuilder<ShopBloc, ShopState>(
                ...
                if (shopState is ShopLoaded) {
                  shopName =
                      shopState.shop.name.isEmpty ? 'Loja' : shopState.shop.name;
                  address1 = shopState.shop.addressLine1;
                  address2 = shopState.shop.addressLine2;
                  phone = shopState.shop.phoneNumber;
                  footer = shopState.shop.footerText;
                  nif = shopState.shop.nif;
                }
                                    ...
                                    ...billingState.cartItems.map((item) {
                                      return TableRow(
                                        children: [
                                          ...
                                          _buildDataCell(AppCurrency.format(item.product.price), TextAlign.right, isSubtitle: true),
                                          _buildDataCell(AppCurrency.format(item.total), TextAlign.right, isBold: true),
                                        ],
                                      );
                                    }),
                                  ],
                              ...
                              PrimaryButton(
                                onPressed: () {
                                  context.read<BillingBloc>().add(PrintReceiptEvent(
                                    shopName: shopName, address1: address1, address2: address2, phone: phone, footer: footer, nif: nif
                                  ));
                                },
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Text(AppCurrency.format(billingState.totalAmount), style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Color(0xFF0F172A))),
                        ],
                      ),
                    ),
                  ],
        ...
        children: [
          if (_selectedMethod == 'Express') ...[
            ...
            if (_cashController.text.isNotEmpty) ...[
              Builder(
                builder: (context) {
                  final given = AppCurrency.parse(_cashController.text) ?? 0;
                  final change = given - totalAmount;
                  return Text('Troco: ${AppCurrency.format(change)}',
