...
class BillingBloc extends Bloc<BillingEvent, BillingState> {
  ...
  Future<void> _onScanBarcode(
    ...
    result.fold(
      (failure) =>
          emit(state.copyWith(
              error: 'Produto não encontrado: ${event.barcode}')),
      (product) {
  ...
  Future<void> _onPrintReceipt(
    ...
    if (!printerHelper.isConnected) {
      ...
      if (savedMac != null) {
        ...
        if (!connected) {
          emit(state.copyWith(
              error: 'Não foi possível ligar automaticamente à impressora.',
              clearError: false));
        ...
        emit(state.copyWith(
            error:
                'Nenhuma impressora ligada nem guardada. Configure em Definições.',
            clearError: false));
        emit(state.copyWith(clearError: true));
        return;
      }
  ...
  Future<void> _onProcessPayment(ProcessPaymentEvent event, Emitter<BillingState> emit) async {
    ...
    // O ecrã de pagamento envia 'Numerário'; 'Cash' fica aceite para não
    // partir vendas gravadas antes da tradução.
    if (state.paymentMethod == 'Numerário' || state.paymentMethod == 'Cash') {
      double amountGiven = AppCurrency.parse(
              event.paymentData['amount_given'].toString()) ??
          0;
  ...
  Future<void> _onConfirmPayment(ConfirmPaymentEvent event, Emitter<BillingState> emit) async {
    try {
      ...
      final transaction = AccountingTransaction(
        ...
        description: 'Venda - ${state.paymentMethod}',
      ...
      emit(state.copyWith(error: 'Falha ao confirmar o pedido: $e'));
    }
  }
  ...
  Future<void> _onSharePdfReceipt(SharePdfReceiptEvent event, Emitter<BillingState> emit) async {
    ...
    try {
      ...
      emit(state.copyWith(
          isPrinting: false, error: 'Falha ao partilhar o PDF: $e'));
    }
  }
}
