...
class PrinterHelper {
  ...
  Future<void> printReceipt({
    ...
    bytes += EscPos.lineFeed;
    if (nif.isNotEmpty) {
      bytes += _textToBytes('NIF: $nif');
      bytes += EscPos.lineFeed;
    }

    // Data e hora (formato de 24 horas, como se usa em Angola)
    ...
    bytes += _textToBytes('Artigo          Preco    Total');
    bytes += EscPos.lineFeed;
    bytes += _textToBytes('--------------------------------');
    bytes += EscPos.lineFeed;
    ...
    for (var item in items) {
      ...
      String price = AppCurrency.formatPlain(item['price'] as num);
      String totalItem = AppCurrency.formatPlain(item['total'] as num);

      String prefix = '${qty}x $name';
      if (prefix.length > 16) prefix = prefix.substring(0, 16);

      String line = prefix.padRight(16) + price.padLeft(8) + totalItem.padLeft(9);
    ...
    bytes += EscPos.boldOn;
    bytes += _textToBytes('TOTAL: ${AppCurrency.format(total)}');
    bytes += EscPos.lineFeed;
    bytes += EscPos.boldOff;
    bytes += EscPos.lineFeed;
  ...
  /// Remove os acentos antes de imprimir.
  ///
  /// As impressoras térmicas usam por omissão uma tabela de caracteres (CP437)
  /// onde os bytes acentuados do português dão glifos errados — "Ã§" em vez de
  /// "ç". Trocar por ASCII mantém o talão legível em qualquer impressora.
  static String _foldAccents(String text) {
    const from = 'áàâãäéèêëíìîïóòôõöúùûüçÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇºª';
    const to = 'aaaaaeeeeiiiiooooouuuucAAAAAEEEEIIIIOOOOOUUUUCoa';
  ...
  List<int> _textToBytes(String text) {
    return List.from(_foldAccents(text).codeUnits);
  }
}
